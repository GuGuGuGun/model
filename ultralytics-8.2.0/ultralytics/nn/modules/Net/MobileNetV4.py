...
class MobileLayer1_small(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.layer_data = {
            ...
            "num_blocks": 2,
            "block_specs": [
                [self.in_channel, 32, 3, 2],
                [32, out_channel, 1, 1]
            ]
        }
        self.layer = build_blocks(self.layer_data)

    def forward(self, x):
        return self.layer(x)


class MobileLayer2_small(nn.Module):
    def __init__(self, in_channel, out_channel):
        super().__init__()
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.layer_data = {
            ...
            "num_blocks": 2,
            "block_specs": [
                [self.in_channel, 96, 3, 2],
                [96, self.out_channel, 1, 1]
            ]
        }
        self.layer = build_blocks(self.layer_data)
...
class MobileLayer3_small(nn.Module):
    def __init__(self, in_channel, out_channel):
        super().__init__()
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.layer_data = {
            ...
            "num_blocks": 6,
            "block_specs": [
                [self.in_channel, 96, 5, 5, True, 2, 3],
                [96, 96, 0, 3, True, 1, 2],
                [96, 96, 0, 3, True, 1, 2],
                [96, 96, 0, 3, True, 1, 2],
                [96, 96, 0, 3, True, 1, 2],
                [96, self.out_channel, 3, 0, True, 1, 4],
            ]
        }
        self.layer = build_blocks(self.layer_data)
...
class MobileLayer4_small(nn.Module):
    def __init__(self, in_channel, out_channel):
        super().__init__()
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.layer_data = {
            ...
            "num_blocks": 6,
            "block_specs": [
                [self.in_channel, 128, 3, 3, True, 2, 6],
                [128, 128, 5, 5, True, 1, 4],
                [128, 128, 0, 5, True, 1, 4],
                [128, 128, 0, 5, True, 1, 3],
                [128, 128, 0, 3, True, 1, 4],
                [128, self.out_channel, 0, 3, True, 1, 4],
            ]
        }
        self.layer = build_blocks(self.layer_data)
...
class MobileLayer5_small(nn.Module):
    def __init__(self, in_channel, out_channel):
        super().__init__()
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.layer_data = {
            ...
            "num_blocks": 2,
            "block_specs": [
                [self.in_channel, 960, 1, 1],
                [960, self.out_channel, 1, 1]
            ]
        }
        self.layer = build_blocks(self.layer_data)
...
class MobileConv(nn.Module):
    def __init__(self, in_channel, out_channel, model):
        ...
        self.layer_data = self.spec["conv0"]
        self.layer_data["block_specs"][0][0] = self.in_channel
        self.layer_data["block_specs"][0][1] = self.out_channel
        self.layer = build_blocks(self.layer_data)
...
class MobileLayer1(nn.Module):
    def __init__(self, in_channel, out_channel, model):
        ...
        self.model = model
        self.spec = MODEL_SPECS[self.model]
        self.layer_data = self.spec['layer1']
        self.layer_data["block_specs"][0][0] = self.in_channel
        self.layer_data["block_specs"][-1][1] = self.out_channel
        self.layer = build_blocks(self.layer_data)
...
class MobileLayer2(nn.Module):
    def __init__(self, in_channel, out_channel, model):
        ...
        self.model = model
        self.spec = MODEL_SPECS[self.model]
        self.layer_data = self.spec['layer2']
        self.layer_data["block_specs"][0][0] = self.in_channel
        self.layer_data["block_specs"][-1][1] = self.out_channel
        self.layer = build_blocks(self.layer_data)
...
class MobileLayer3(nn.Module):
    def __init__(self, in_channel, out_channel, model):
        ...
        self.model = model
        self.spec = MODEL_SPECS[self.model]
        self.layer_data = self.spec['layer3']
        self.layer_data["block_specs"][0][0] = self.in_channel
        self.layer_data["block_specs"][-1][1] = self.out_channel
        self.layer = build_blocks(self.layer_data)
...
class MobileLayer4(nn.Module):
    def __init__(self, in_channel, out_channel, model):
        ...
        self.model = model
        self.spec = MODEL_SPECS[self.model]
        self.layer_data = self.spec['layer4']
        self.layer_data["block_specs"][0][0] = self.in_channel
        self.layer_data["block_specs"][-1][1] = self.out_channel
        self.layer = build_blocks(self.layer_data)
...
class MobileLayer5(nn.Module):
    def __init__(self, in_channel, out_channel, model):
        ...
        self.model = model
        self.spec = MODEL_SPECS[self.model]
        self.layer_data = self.spec['layer5']
        self.layer_data["block_specs"][0][0] = self.in_channel
        self.layer_data["block_specs"][-1][1] = self.out_channel
        self.layer = build_blocks(self.layer_data)
...
if __name__ == '__main__':
    ...
    # layerConv = MobileConv(3, 64, 'MobileNetV4ConvSmall')
    layer1 = MobileLayer1(64, 128, 'MobileNetV4ConvSmall')
    # layer2 = MobileLayer2(128, 256, 'MobileNetV4ConvSmall')
    # layer3 = MobileLayer3(256, 512, 'MobileNetV4ConvSmall')
    # layer4 = MobileLayer4(512, 1024, 'MobileNetV4ConvSmall')
    # layer5 = MobileLayer5(1024, 1024, 'MobileNetV4ConvSmall')
    # layer = [layerConv,layer1,layer2,layer3,layer4,layer5]
    # for i in layer:
    #     t1 = i(t1)
    #     print(t1.shape)
    print(layer1)
