...
def model_latency(model, device, example_inputs, iterations=1000):
    """测试模型的推理延迟。"""
...
def func(opt):
    ...
    model = YOLO('D:\deeplearning/ultralytics-8.2.0/ultralytics\cfg\models/v8/yolov8.yaml').load(weights)
    model = model.to(device)
    rand = torch.randn((opt.batch, 3, *opt.imgs))
    mean = torch.mean(rand,dim=0, keepdim=True)
    std = torch.std(rand,dim=0, keepdim=True)
    example_inputs = ((rand-mean)/std).to(device)
    ...
    mean_latency, std_latency = model_latency(model, device, example_inputs, opt.testtime)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='D:\deeplearning\yolo8-test\\train\DET\little_dataset\\100\\no\weights\\best.pt', help='trained weights path')
